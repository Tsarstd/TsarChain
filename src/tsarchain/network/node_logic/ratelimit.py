...
def is_banned(ip: str, now_ts: float | None = None, *, node_id: str | None = None) -> bool:
...
def ban_peer(ip: str, seconds: float, *, node_id: str | None = None) -> None:
    if not ip:
        return
    if ip in ("127.0.0.1", "::1"):
        return
...
def allow_handshake(
    ...
    *,
    ...
    if is_banned(ip, now_ts, node_id=node_id):
        return False
    ...
    if pow_proof:
        ...
        pow_ok = verify_pow(pow_proof, nonce, expected_scope=scope, identity=identity_key or f"ip:{ip}")
        if pow_ok:
            return True
    ...
    if identity_key:
        if not _hit(_handshake_hits_id, identity_key, id_window, id_burst, now_ts):
            ban_peer(ip, CFG.TEMP_BAN_SECONDS, node_id=identity_key)
            return False
    ...
    if not _hit(_handshake_hits_ip, ip, ip_window, ip_burst, now_ts):
        if identity_key:
            ...
            ban_peer(ip, CFG.TEMP_BAN_SECONDS)
        return False
