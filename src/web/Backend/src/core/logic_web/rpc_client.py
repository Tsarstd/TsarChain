...
log = get_ctx_logger("tsarchain.web.Backend.src.core.logic_web.rpc_client")
...
def drop_client(host: str, port: int) -> None:
    key = f"{host}:{port}"
    with _CLIENT_LOCK:
        _CLIENT_CACHE.pop(key, None)


def rpc_send(client, payload: dict):
    if RPC_SOURCE and payload and "rpc_source" not in payload:
        payload = dict(payload)
        payload["rpc_source"] = RPC_SOURCE
    return client.send(payload)


def set_cache_scope(host: str, port: int) -> None:
    global _CACHE_SCOPE
...
def cache_get(key: str, refresh_ttl: bool = False):
    return db_cache.cache_get(key, refresh_ttl=refresh_ttl)


def cache_set(key: str, payload: object, ttl_sec: int | None = None) -> None:
    if ttl_sec is None:
...
def get_or_fetch_cached(key: str, fetch_fn):
    ...
    if cache_ok:
        cache_set(key, payload, ttl_sec)
    return payload


def payload_has_error(payload: object) -> bool:
    return bool(payload and payload.get("error"))
...
def _determine_cache_policy(payload: object) -> Tuple[bool, int | None]:
    ...
    if err:
        ttl = db_cache.get_error_cache_ttl(err or payload.get("detail") or payload.get("reason"))
