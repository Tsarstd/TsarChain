export const architecture = {
  ...
  toc: [
    ...
    { id: "consensus-pow", label: "Consensus & Proof-of-Work (PoW) Logic" },
    ...
    { id: "graffiti-protocol-logic", label: "Graffiti Protocol Logic" },
  ],
  sections: [
    {
      id: "overview",
...
└── tests/                      # All Python Unit testing`
    ...
    },
    {
      ...
      content: `The examples below illustrate real live serialized states across blocks, UTXOs, mempool, Graffiti activities, metadata registry, storage indexer, and state database. Click **"Show Preview"** on any item to inspect its complete JSON payload:

#### 💸 Example of a Transaction Data Structure

The example data of \`block, utxo's & mempool\` below, shows a transaction :

> address \`tsar1qakf6mle606amn7xumvz4k6yu6cz0mxq6pe5qwr\` sent 2300 coins + 0.00004900 fee to address \`tsar1qn76f5d32xe9405hsteujjyyuahrcynh5cxjw23\`, then validated by miner \`tsar1qyekatj93lvaq6xr3q9r8zwhkdzmk9hr0fmfrss\` in block height 10

#### 🎨 Example of a Graffiti Activity Data Structure

The example data of \`block (height 18), block (height 22), block (height 29), metadata & indexer\` below, shows graffiti activity :

> creator \`tsar1qakf6mle606amn7xumvz4k6yu6cz0mxq6pe5qwr\` uploads graffiti art (80,358 bytes) and pays 0.8 coins to the network, validated in \`block height 18\`

> then commenter \`tsar1qn76f5d32xe9405hsteujjyyuahrcynh5cxjw23\` comments on the graffiti art with 1 coin + 2 coins creator tip, validated in \`block height 22\`

> archivist \`tsar1qzxzzf5az5guk43mf0z4d94uugat4jcejwglp07\` (storage node) receives payouts of 0.6 coin from the pool address \`tsar1qm0vgpssx8w95ddktmfg86ahhy863nu2u42w5qceh3n24xkvk83mqqa2jng\`, validated in \`block height 29\`

#### 🌐 Example of a State Data Structure

State is the database for overall network consensus statistics, supply emissions, and snapshot metrics.

> ℹ️ All data structures below represent the streaming and fixed LMDB storage models exported via \`tools/lmdb_to_json.py\`. The production storage model is pure binary in \`.mdb\` LMDB environments.`,
      dataStructures: [
        {
          id: "tx-block-data",
          title: "1. Block Data Structure (chain.json / LMDB chain)",
  ...
  "h:000000000010": {
    ...
    "transactions": [
      ...
      },
      {
        ...
        {
          id: "tx-utxo-data",
          title: "2. UTXO's Data Structure (utxo.json / LMDB utxo)",
  ...
  "88bc192050945f1723ab416d8c53bc1b332d2d8fa737d3dfb26c931df9fc04b5:1": {
    ...
    "script_type": "p2wpkh"
        ...
        {
          id: "tx-mempool-data",
          title: "3. MemPool Data Structure (mempool.json / LMDB mempool)",
          subtitle: "Compact binary header (<dIII) + serialized witness Tx decoded entries",
          code: `{
  "88bc192050945f1723ab416d8c53bc1b332d2d8fa737d3dfb26c931df9fc04b5": {
    "fee": 4900,
    ...
    "weight": 820
        ...
        },
        {
          ...
          title: "4. Block (graffiti post - height 18) Data Structure (chain.json)",
          subtitle: "Block height 18 validating Graffiti POST transaction, art_id anchoring, and pool output",
          code: `{
  "h:000000000018": {
    "bits": 521525829,
    ...
    "transactions": [
      ...
      },
      {
        ...
        },
        {
          ...
          title: "5. Block (comment - height 22) Data Structure (chain.json)",
          subtitle: "Block height 22 validating Graffiti COMMENT transaction, creator royalty & pool split",
          code: `{
  "h:000000000022": {
    "bits": 521422584,
    ...
    "transactions": [
      ...
      },
      {
        ...
        },
        {
          ...
          title: "6. Block (payouts - height 29) Data Structure (chain.json)",
          subtitle: "Block height 29 validating Graffiti PAYOUT claim transaction to storage node",
          code: `{
  "h:000000000029": {
    "bits": 521872887,
    ...
    "transactions": [
      ...
      },
      {
        ...
        {
          id: "graffiti-metadata-structure",
          title: "7. Graffiti (metadata) Structure (graffiti/ sub-files)",
          subtitle: "Granular isolated binary databases for posts, comments, payouts, and retention proofs",
          code: `{
  "comments": {
    ...
    "grafd99f16dde43a8c2740396ba6a71d8f1666612c4a9790d7060edfe236cfd8": [
      {
        ...
        "epoch": 0,
        "recipients": {
          "tsar1qzxzzf5az5guk43mf0z4d94uugat4jcejwglp07": 60000000
        },
        "txid": "cf5a20bfc3ac20250b1556cad466c4d1a6e937a6e7c8f645a6ff961e78dfe46d"
      }
    ]
    ...
    "grafd99f16dde43a8c2740396ba6a71d8f1666612c4a9790d7060edfe236cfd8": [
      ...
      },
      {
        ...
        },
        {
          ...
          title: "8. Graffiti (indexer) in Storage Node (index.json)",
  ...
  "files": {
    "9b29cf795af60e77ee707df235f276e825fc0ee0f0c75b8b76d0dd29f7cdf701_1764670929": {
      ...
      "updated_ts": 1764670929
        ...
        {
          id: "graffiti-utxo-activity",
          title: "9. UTXO data when graffiti activity occurs (.json)",
          subtitle: "UTXO state transitions during comment activity and archivist payouts",
          code: `// This the flow when citizen start put a comment in graffiti
...
"b5b7cf44c7377ce91b324b5b1b1d6e9baad5716c4b5905607b2a8b6175ee4b33:0": {
  ...
  "script_type": "p2wpkh"
        ...
        },
        {
          ...
          title: "10. State Data Structure (state.json / LMDB state)",
  ...
  "k:total_blocks": 32,
  "k:total_supply": 250775000000000
}`
        }
      ]
    },
    {
      id: "consensus-pow",
      title: "Consensus & Proof-of-Work (PoW) Logic",
...
- **Standard UTXOs**: Standard financial/token transfers, which are sorted by \`fee\` (descending) and then \`_received_at\`.
...
> **1 BLOCK = 1 GRAFFITI**
> To prevent spam and maintain the semantic weight of each block, the \`_build_candidate_block\` function enforces a strict quota. A candidate block **may only contain a maximum of 1 Graffiti POST transaction**. If multiple \`POST\` transactions exist in the mempool, the orchestrator accepts the first one and explicitly skips the rest, leaving them queued for subsequent blocks.
...
- **Storage & State** (\`chain_ops.py\` & \`chain_storage.py\`): Valid blocks are committed to the LMDB database. The protocol correctly applies UTXO deltas and can orchestrate complex chain re-organizations (swapping and replacing blocks) if a longer valid chain is encountered.`
    },
    {
      ...
      content: `The ledger state of the Graffiti Protocol tracks ownership, storage balances, and digital artifact lifecycles through a highly optimized UTXO (Unspent Transaction Output) framework.
...
- **Mempool Soft Policies**: While consensus limits block-inclusion to 1 Graffiti POST per block, the mempool also implements soft limits (\`_enforce_mempool_post_limit\`) to reject overflow posts and protect the node's memory capacity.`
    },
    {
      ...
      content: `For performance-critical tasks, the Graffiti Protocol bypasses the performance limitations and the Global Interpreter Lock (GIL) of Python by offloading computations to a high-performance native extension compiled in Rust.
...
- \`graff_merkle_verify\`: Instantly validates a Proof of Retention challenge, ensuring nodes cannot easily fake possession of archived files.`
    },
    {
      ...
      title: "Graffiti Protocol Logic",
      content: `The core logic of the Graffiti Protocol governs the creation, validation, storage pool distributions, and retention auditing of all cultural archives in the system.
...
**Deterministic P2WSH Address Pools**: Each piece of digital art is tied to a deterministic Pay-to-Witness-Script-Hash (P2WSH) address derived from \`_pool_redeem_script(art_id_hex)\`.
  ...
  - Coin balances built up inside this pool address are locked.
