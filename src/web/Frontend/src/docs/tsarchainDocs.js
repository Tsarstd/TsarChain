...
- **Logarithmic Verification**: Merkle proofs verify random byte chunks in \`O(log N)\` complexity without loading multi-gigabyte files into RAM.`
  ...
  api: {
    id: "api",
    title: "API & Socket Protocol",
    subtitle: "JSON-RPC over TCP Socket Protocol",
    category: "Tsarchain",
    badge: "Socket RPC",
    toc: [
      { id: "protocol-overview", label: "1. Protocol Overview" },
      { id: "security-pow-token", label: "2. Security & Anti-Spam (PoW Token)" },
      { id: "user-rpc", label: "3. User RPC Category" },
      { id: "miner-rpc", label: "4. Miner RPC Category" },
      { id: "storage-rpc", label: "5. Storage Node RPC Category" },
      { id: "error-handling", label: "6. Error Codes & Payloads" },
    ],
    sections: [
      {
        id: "protocol-overview",
        title: "1. Protocol Overview",
        content: `Communication between nodes, wallets, storage entities, and explorers operates entirely via JSON-RPC payloads transmitted over a raw TCP socket.

\`\`\`json
{
  "type": "ACTION_TYPE",
  "client": "kremlin_wallet_v1",
  "payload": { ... }
}
\`\`\`

The node dispatcher categorizes requests into three authorization roles: **USER**, **MINER**, or **STORAGE**.`
      },
      {
        id: "security-pow-token",
        title: "2. Security & Anti-Spam (PoW Token)",
        content: `To prevent DDoS floods while remaining stateless:
1. Server issues a challenge token (\`issue_pow\`) requiring target difficulty.
2. Client computes a nonce (\`solve_pow\`) resulting in required leading zero bits.
3. Server validates the token instantly (\`verify_pow\`) without maintaining memory state.`
      },
      {
        id: "user-rpc",
        title: "3. User RPC Category",
        content: `Public-facing API category for wallets and explorers:

- \`GET_BALANCES\`: Query UTXO balances for an address.
- \`CREATE_TX\`: Construct a standard P2WPKH transfer.
- \`CREATE_TX_MULTI\`: Construct POST, COMMENT, or PAYOUT transaction with OP_RETURN payload.
- \`NEW_TX\`: Broadcast signed transaction to mempool.
- \`GET_BLOCK\`: Retrieve full block details by height or block hash.
- \`GET_TX_DETAIL\`: Query transaction metadata and confirmation depth.
- \`GET_GRAFFITI_POSTS\`: Fetch indexed graffiti artifacts.

\`\`\`json
// Example: GET_BALANCES request
{
  "type": "GET_BALANCES",
  "address": "tsar1qakf6mle606amn7xumvz4k6yu6cz0mxq6pe5qwr",
  "client": "web_explorer"
...
\`\`\``
      ...
      {
        id: "miner-rpc",
        title: "4. Miner RPC Category",
        content: `- \`GET_BLOCK_TEMPLATE\`: Request mining candidate block containing prioritized transactions and RandomX seed.
- \`SUBMIT_BLOCK\`: Broadcast a solved PoW nonce and block header.`
      },
      {
        id: "storage-rpc",
        title: "5. Storage Node RPC Category",
        content: `- \`STOR_INIT\`: Register intent to store a graffiti artifact.
- \`STOR_COMMIT\`: Finalize file chunk upload and generate Merkle root.
- \`STOR_PROOF_RUN\`: Submit PoR cryptographic challenge response.
- \`STOR_PAYOUT_CLAIM\`: Claim epoch rewards from the deterministic P2WSH pool.`
      },
      {
        id: "error-handling",
        title: "6. Error Codes & Payloads",
        content: `Standard response structure:
\`\`\`json
{
  "status": "error",
  "error_code": "MEMPOOL_POLICY_VIOLATION",
  "message": "Transaction exceeds max vbytes allowed in current mempool configuration"
}
\`\`\``
      }
    ]
