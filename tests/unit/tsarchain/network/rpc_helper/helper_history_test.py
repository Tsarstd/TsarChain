...
# ----------------------------------------------------------------------
# Tests for is_coinbase_tx
# ----------------------------------------------------------------------
def test_is_coinbase_tx_no_inputs(mixin):
    tx = DummyTx(inputs=[])
    assert mixin.is_coinbase_tx(tx) is True
...
def test_is_coinbase_tx_non_coinbase(mixin):
    ...
    tx = DummyTx(inputs=[tin])
    assert mixin.is_coinbase_tx(tx) is False
...
def test_txout_to_address(mixin):
    ...
    with patch('tsarchain.network.rpc_helper.history.spkhex_to_address', return_value='tsar1abc') as mock_spk:
        result = mixin.txout_to_address(txout)
...
def test_normalize_spk_hex_bech32(mixin):
    addr = 'tsar1q...'  # dummy
    with patch.object(mixin, 'addr_to_spk') as mock_addr_to_spk:
        mock_spk = Mock()
...
def test_build_outpoint_map_chain_only(mixin):
    ...
    chain = [block]
    result = mixin.build_outpoint_map(chain)
    assert len(result) == 3
    key1 = tx1.txid.hex() + ':0'
    key2 = tx1.txid.hex() + ':1'
    key3 = tx2.txid.hex() + ':0'
...
def test_build_outpoint_map_with_mem(mixin):
    ...
    chain_map, mem_map = mixin.build_outpoint_map(chain, mem)
...
def test_find_tx_and_meta_in_mempool(mixin):
    ...
    result = mixin.find_tx_and_meta(txid_hex)
...
def test_find_tx_and_meta_not_found(mixin):
    ...
    result = mixin.find_tx_and_meta('deadbeef')
...
# ----------------------------------------------------------------------
# Tests for process_history_lookup (integration)
# ----------------------------------------------------------------------
def test_process_history_lookup_invalid_address(mixin):
    result = mixin.process_history_lookup('')
    assert result == {'items': [], 'total': 0, 'limit': 50, 'offset': 0}
    result = mixin.process_history_lookup(None)
    assert result == {'items': [], 'total': 0, 'limit': 50, 'offset': 0}

def test_process_history_lookup_address_too_long(mixin, patch_config):
    patch_config.MAX_UTXO_ADDR_LEN = 5
    result = mixin.process_history_lookup('a'*10)
    assert 'error' in result
    assert result['error'] == 'address too long'

def test_process_history_lookup_invalid_address_format(mixin):
    with patch.object(mixin, '_normalize_spk_hex', return_value=None):
        result = mixin.process_history_lookup('invalid')
        assert 'error' in result
        assert result['error'] == 'invalid address'

def test_process_history_lookup_cache_hit(mixin):
    ...
    with patch.object(mixin, 'bhash_hex', return_value='abc'):
        with patch.object(mixin, 'build_outpoint_map') as mock_build:
            result = mixin.process_history_lookup('0014' + 'a'*40)
            assert len(result['items']) == 1
            assert result['total'] == 1
            mock_build.assert_not_called()  # cache hit, no rebuild

def test_process_history_lookup_cache_miss(mixin):
    target_spk = '0014' + 'a'*40
    ...
    with patch.object(mixin, '_normalize_spk_hex', return_value=target_spk):
        ...
        opmap_mem = {}
        with patch.object(mixin, 'build_outpoint_map', return_value=(opmap_chain, opmap_mem)):
...
def test_process_history_lookup_coinbase_incoming(mixin):
    target_spk = '0014' + 'a'*40
...
def test_process_history_lookup_deduplication(mixin):
    txid_hex = 'd'*64
    tx = DummyTx(
        txid=bytes.fromhex(txid_hex),
        inputs=[DummyTxIn(txid=b'p'*32, vout=0)],
