# Graffiti Protocol - API & RPC Documentation

This document serves as the primary reference for developers building on or integrating with the Graffiti Protocol node network. Communication between nodes, wallets, storage entities, and explorers operates entirely via JSON-RPC payloads transmitted over a raw TCP socket.


## 1. Network & Socket Protocol Overview
...
## 2. Security & Anti-Spam (Rate Limiting & PoW)

To maintain a durable and attack-resistant decentralized network, Graffiti Protocol enforces strict rate limits and utilizes a Proof-of-Work (PoW) token challenge.
...
1. The server issues a challenge token (`issue_pow`) requiring a specific `difficulty` level.
2. The client calculates a `nonce` (`solve_pow`) that results in a SHA-256 hash containing a sufficient number of leading zero bits.
3. The server instantly verifies the `nonce` (`verify_pow`) without keeping it in memory. This effectively mitigates DDoS attacks while preserving server state.
...
### A. User RPC (`USER`)
This is the public-facing API category used by wallets, blockchain explorers, and social chat apps. It handles transactions, fetching blocks, and posting Graffiti art.

> 📖 **Full Technical Specification**: [USER_RPC.MD](../src/tsarchain/network/rpc/docs/USER_RPC.MD)
...
> 📖 **Full Technical Specification**: [MINER_RPC.MD](../src/tsarchain/network/rpc/docs/MINER_RPC.MD)
...
### C. Storage Node RPC (`STORAGE`)
Specialized endpoints for nodes that participate in archiving the heavy digital media elements of Graffiti.

> 📖 **Full Technical Specification**: [STORAGE_RPC.MD](../src/tsarchain/network/rpc/docs/STORAGE_RPC.MD)
...
### D. Archivist (Storage Layer) RPC
These RPCs are specifically served by Storage Nodes (Archivists) to handle direct file uploads/downloads from users, and to communicate retention proofs with the main network.
